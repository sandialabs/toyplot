...
def render(canvas, fobj=None, animation=False):
  ...
  if len(svg_animation) > 1:
    ...
    xml.SubElement(controls, "script").text = _animation_controls.substitute(root_id=root.get("id"), frame_durations=json.dumps(durations.tolist()), state_changes=json.dumps(changes))
...
def _render(canvas, axes, context):
  ...
  for cell in axes._visible_cells:
    if cell._bstyle is not None:
      cell_xml = xml.SubElement(axes_xml, "rect", x=repr(cell._left), y=repr(cell._top), width=repr(cell._right - cell._left), height=repr(cell._bottom - cell._top), style=_css_style(cell._bstyle))
