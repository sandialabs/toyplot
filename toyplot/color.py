...
class Palette(object):
    ...
    def _repr_html_(self):
        root_xml = xml.Element("div", style="overflow:hidden; height:auto", attrib={
                               "class": "toyplot-color-Palette"})
        for color in self._colors:
            xml.SubElement(
                root_xml, "div", style="float:left;width:20px;height:20px;background-color:%s" % to_css(color))
...
class DivergingMap(object):
    ...
    def colors(self, values, domain_min=None, domain_max=None):
        ...
        for index, amount in enumerate(flat):
            ...
            result[index] = lab(
                *_msh_to_lab(_mix(a[0], b[0], amount), _mix(a[1], b[1], amount), _mix(a[2], b[2], amount)))
        return result.reshape(values.shape)
    ...
    def _repr_html_(self):
        ...
        for color in self.colors(numpy.linspace(domain_min, domain_max, 100, endpoint=True)):
            xml.SubElement(
                root_xml, "div", style="float:left;width:1px;height:20px;background-color:%s" % to_css(color))
        return xml.tostring(root_xml, method="html")
...
class LinearMap(object):
    ...
    def _repr_html_(self):
        ...
        for color in self.colors(numpy.linspace(domain_min, domain_max, sample_count, endpoint=True)):
            xml.SubElement(
                root_xml, "div", style="float:left;width:%spx;height:20px;background-color:%s" % (sample_width, to_css(color)))
        return xml.tostring(root_xml, method="html")
