...
def parse(css):
  ...
  """
  if css.lower() in parse.names:
    color = parse.names[css.lower()]
  ...
  match = parse.hsla(css)
  if match:
    h, s, l, a = [float(group) for group in match.groups()]
    r, g, b = colorsys.hls_to_rgb((h / 360.0) % 1, l / 100.0, s / 100.0)
    return toyplot.color.rgba(r, g, b, a)

parse.hex3 = re.compile("^#([\da-f])([\da-f])([\da-f])$", re.I).match
parse.hex6 = re.compile("^#([\da-f]{2})([\da-f]{2})([\da-f]{2})$", re.I).match
parse.rgb = re.compile("rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\)").match
parse.rgb_percent = re.compile("rgb\(\s*(.*)%\s*,\s*(.*)%\s*,\s*(.*)%\)").match
parse.rgba = re.compile("rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(.*)\)").match
parse.rgba_percent = re.compile("rgba\(\s*(.*)%\s*,\s*(.*)%\s*,\s*(.*)%,\s*(.*)\)").match
parse.hsl = re.compile("hsl\(\s*(.*)\s*,\s*(.*)%\s*,\s*(.*)%\)").match
parse.hsla = re.compile("hsla\(\s*(.*)\s*,\s*(.*)%\s*,\s*(.*)%,\s*(.*)\)").match
