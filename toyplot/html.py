...
def render(canvas, fobj=None, animation=False):
    ...
    svg = xml.Element("svg", xmlns="http://www.w3.org/2000/svg", attrib={"xmlns:toyplot": "http://www.sandia.gov/toyplot"},
                      width="%rpx" % canvas._width, height="%rpx" % canvas._height, style=_css_style(canvas._style), id=context.get_id(canvas))
    ...
    root = xml.Element(
        "div", align="center", attrib={"class": "toyplot"}, id="t" + uuid.uuid4().hex)
    root.append(svg)
    ...
    xml.SubElement(mark_popup, "li", attrib={
                   "class": "toyplot-mark-popup-title"}, style="color:lightgray;cursor:default;padding:5px;list-style:none;margin:0;")
    xml.SubElement(mark_popup, "li", attrib={"class": "toyplot-mark-popup-save-csv"}, style="border-radius:3px;padding:5px;list-style:none;margin:0;",
                   onmouseover="this.style.color='steelblue';this.style.background='white'", onmouseout="this.style.color='white';this.style.background='steelblue'").text = "Save as .csv"

    # Add a workaround for browsers that don't support CSS alignment-baseline.
    if svg.find(".//text") is not None:
        xml.SubElement(controls, "script").text = _alignment_baseline_workaround.substitute(
            root_id=root.get("id"))
    ...
    if context._data_tables:
        ...
        xml.SubElement(controls, "script").text = _export_data_tables.substitute(
            root_id=root.get("id"), data_tables=json.dumps(data_tables))
    ...
    if context._cartesian_axes:
        ...
        for key, axes in context._cartesian_axes.items():
            ...
            for segment in axes._xprojection._segments:
                cartesian_axes[key]["x"].append({"scale": segment.scale, "domain": {"min": segment.domain.min, "max": segment.domain.max, "bounds": {"min": segment.domain.bounds.min, "max": segment.domain.bounds.max}}, "range": {
                                                "min": segment.range.min, "max": segment.range.max, "bounds": {"min": segment.range.bounds.min, "max": segment.range.bounds.max}}})
            cartesian_axes[key]["y"] = list()
            for segment in axes._yprojection._segments:
                cartesian_axes[key]["y"].append({"scale": segment.scale, "domain": {"min": segment.domain.min, "max": segment.domain.max, "bounds": {"min": segment.domain.bounds.min, "max": segment.domain.bounds.max}}, "range": {
                                                "min": segment.range.min, "max": segment.range.max, "bounds": {"min": _flip_infinities(segment.range.bounds.min), "max": _flip_infinities(segment.range.bounds.max)}}})

        xml.SubElement(controls, "script").text = _show_mouse_coordinates.substitute(root_id=root.get(
            "id"), cartesian_axes=json.dumps(cartesian_axes, cls=_NumpyJSONEncoder, sort_keys=True))
    ...
    if len(svg_animation) > 1:
        ...
        xml.SubElement(vcr_controls, "input", title="Frame", type="range", min="0", max=str(
            len(times) - 2), step="1", value="0", id="%s-current-frame" % root.get("id"))
        xml.SubElement(vcr_controls, "button", title="Rewind", style="width:40px;height:24px", id="%s-rewind" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><polygon points="10,5 0,10 10,15" stroke="none" fill="{near_black}"/><polygon points="20,5 10,10 20,15" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))
        xml.SubElement(vcr_controls, "button", title="Reverse Play", style="width:40px;height:24px", id="%s-reverse-play" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><polygon points="15,5 5,10 15,15" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))
        xml.SubElement(vcr_controls, "button", title="Frame Rewind", style="width:40px;height:24px", id="%s-frame-rewind" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><polygon points="15,5 5,10 15,15" stroke="none" fill="{near_black}"/><rect x="17" y="5" width="2" height="10" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))
        xml.SubElement(vcr_controls, "button", title="Stop", style="width:40px;height:24px", id="%s-stop" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><rect x="5" y="5" width="10" height="10" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))
        xml.SubElement(vcr_controls, "button", title="Frame Advance", style="width:40px;height:24px", id="%s-frame-advance" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><polygon points="5,5 15,10 5,15" stroke="none" fill="{near_black}"/><rect x="1" y="5" width="2" height="10" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))
        xml.SubElement(vcr_controls, "button", title="Play", style="width:40px;height:24px", id="%s-forward-play" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><polygon points="5,5 15,10 5,15" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))
        xml.SubElement(vcr_controls, "button", title="Fast Forward", style="width:40px;height:24px", id="%s-fast-forward" % root.get("id")).append(xml.XML(
            """<svg width="20" height="20"><polygon points="0,5 10,10 0,15" stroke="none" fill="{near_black}"/><polygon points="10,5 20,10 10,15" stroke="none" fill="{near_black}"/></svg>""".format(near_black=toyplot.color.near_black)))

        xml.SubElement(controls, "script").text = _animation_controls.substitute(root_id=root.get(
            "id"), frame_durations=json.dumps(durations.tolist()), state_changes=json.dumps(changes))
...
def _render_marker(root, cx, cy, size, marker, marker_style=None, label_style=None, extra_class=None):
    ...
    if shape == "|":
        xml.SubElement(marker_xml, "line", transform="rotate(%r, %r, %r)" % (-shape_angle, cx, cy),
                       x1=repr(cx), x2=repr(cx), y1=repr(cy - (size / 2)), y2=repr(cy + (size / 2)))
    elif shape == "+":
        xml.SubElement(marker_xml, "line", transform="rotate(%r, %r, %r)" % (-shape_angle, cx, cy),
                       x1=repr(cx), x2=repr(cx), y1=repr(cy - (size / 2)), y2=repr(cy + (size / 2)))
        xml.SubElement(marker_xml, "line", transform="rotate(%r, %r, %r)" % (-shape_angle, cx, cy),
                       x1=repr(cx - (size / 2)), x2=repr(cx + (size / 2)), y1=repr(cy), y2=repr(cy))
    elif shape == "*":
        xml.SubElement(marker_xml, "line", transform="rotate(%r, %r, %r)" % (-shape_angle, cx, cy),
                       x1=repr(cx), x2=repr(cx), y1=repr(cy - (size / 2)), y2=repr(cy + (size / 2)))
        xml.SubElement(marker_xml, "line", transform="rotate(%r, %r, %r)" % (-shape_angle + 60,
                                                                             cx, cy), x1=repr(cx), x2=repr(cx), y1=repr(cy - (size / 2)), y2=repr(cy + (size / 2)))
        xml.SubElement(marker_xml, "line", transform="rotate(%r, %r, %r)" % (-shape_angle - 60,
                                                                             cx, cy), x1=repr(cx), x2=repr(cx), y1=repr(cy - (size / 2)), y2=repr(cy + (size / 2)))
    elif shape == "^":
        xml.SubElement(marker_xml, "polygon", transform="rotate(%r, %r, %r)" % (-shape_angle, cx, cy), points=" ".join(
            ["%r,%r" % (xp, yp) for xp, yp in [(cx - (size / 2), cy + (size / 2)), (cx, cy - (size / 2)), (cx + (size / 2), cy + (size / 2))]]))
    elif shape == "s":
        xml.SubElement(marker_xml, "rect", transform="rotate(%r, %r, %r)" % (-shape_angle, cx, cy),
                       x=repr(cx - (size / 2)), y=repr(cy - (size / 2)), width=repr(size), height=repr(size))
    elif shape == "o":
        xml.SubElement(
        ...
        xml.SubElement(marker_xml, "path", transform="translate(%r, %r) scale(%r) translate(%r, %r)" % (
            cx, cy, size, -cx, -cy), d="M " + repr(cx) + " " + repr(cy) + shape_path)

    if shape_label is not None:
        xml.SubElement(marker_xml, "text", x=repr(cx), y=repr(cy), attrib=_css_attrib(
            {"stroke": "none", "fill": toyplot.color.near_black, "text-anchor": "middle", "alignment-baseline": "middle", "font-size": "%rpx" % (size * 0.75)}, label_style, shape_label_style)).text = shape_label
...
def _render(canvas, axes, context):
    ...
    xml.SubElement(clip_xml, "rect", x=repr(axes._xmin_range), y=repr(axes._ymin_range), width=repr(
        axes._xmax_range - axes._xmin_range), height=repr(axes._ymax_range - axes._ymin_range))

    children_xml = xml.SubElement(axes_xml, "g", attrib={
                                  "class": "toyplot-coordinate-events", "clip-path": "url(#%s)" % clip_xml.get("id")}, style=_css_style({"cursor": "crosshair"}))
    xml.SubElement(children_xml, "rect", x=repr(axes._xmin_range), y=repr(axes._ymin_range), width=repr(axes._xmax_range - axes._xmin_range),
                   height=repr(axes._ymax_range - axes._ymin_range), style=_css_style({"visibility": "hidden", "pointer-events": "all"}))
    ...
    if axes.coordinates._show:
        ...
        xml.SubElement(coordinates_xml, "rect", x=repr(axes.coordinates._xmin_range), y=repr(axes.coordinates._ymin_range), width=repr(
            axes.coordinates._xmax_range - axes.coordinates._xmin_range), height=repr(axes.coordinates._ymax_range - axes.coordinates._ymin_range), style=_css_style(axes.coordinates._style))
        xml.SubElement(coordinates_xml, "text", x=repr((axes.coordinates._xmin_range + axes.coordinates._xmax_range) * 0.5), y=repr(
            (axes.coordinates._ymin_range + axes.coordinates._ymax_range) * 0.5), style=_css_style(axes.coordinates.label._style))
    ...
    if axes._show:
        ...
        if axes.x._show:
            if axes.x.spine._show:
                ...
                xml.SubElement(axes_xml, "line", x1=repr(x1), y1=repr(spine_y), x2=repr(
                    x2), y2=repr(spine_y), style=_css_style(axes.x.spine._style))
            ...
            if axes.x.ticks._show:
                ...
                for location, tick_style in zip(axes._xtick_locations, axes.x.ticks.tick.styles(axes._xtick_locations)):
                    ...
                    xml.SubElement(ticks_group, "line", x1=repr(x), y1=repr(y1), x2=repr(
                        x), y2=repr(y2), style=_css_style(axes.x.ticks._style, tick_style))
            ...
            if axes.x.ticks.labels._show:
                ...
                for location, label, title, label_style in zip(axes._xtick_locations, axes._xtick_labels, axes._xtick_titles, axes.x.ticks.labels.label.styles(axes._xtick_locations)):
                    ...
                    dstyle = toyplot.style.combine(
                        {"text-anchor": "middle", "alignment-baseline": "middle", "baseline-shift": "-80%"}, axes.x.ticks.labels._style, label_style)
                    label_xml = xml.SubElement(
                        ticks_group, "text", x=repr(x), y=repr(y), style=_css_style(dstyle))
                    label_xml.text = label
                    if axes.x.ticks.labels._angle:
                        label_xml.set(
                            "transform", "rotate(%r, %r, %r)" % (-axes.x.ticks.labels._angle, x, y))
            ...
            if axes.x.label._text is not None:
                ...
                xml.SubElement(axes_xml, "text", x=repr(x), y=repr(
                    y), style=_css_style(axes.x.label._style)).text = axes.x.label._text
        ...
        if axes.y._show:
            if axes.y.spine._show:
                ...
                xml.SubElement(axes_xml, "line", x1=repr(spine_x), y1=repr(y1), x2=repr(
                    spine_x), y2=repr(y2), style=_css_style(axes.y.spine._style))
            ...
            if axes.y.ticks._show:
                ...
                for location, tick_style in zip(axes._ytick_locations, axes.y.ticks.tick.styles(axes._ytick_locations)):
                    ...
                    xml.SubElement(ticks_group, "line", x1=repr(x1), y1=repr(y), x2=repr(
                        x2), y2=repr(y), style=_css_style(axes.y.ticks._style, tick_style))
            ...
            if axes.y.ticks.labels._show:
                ...
                for location, label, title, label_style in zip(axes._ytick_locations, axes._ytick_labels, axes._ytick_titles, axes.y.ticks.labels.label.styles(axes._ytick_locations)):
                    ...
                    y = axes._project_y(location)
                    dstyle = toyplot.style.combine(
                        {"text-anchor": "middle", "alignment-baseline": "middle", "baseline-shift": "80%"}, axes.y.ticks.labels._style, label_style)
                    label_xml = xml.SubElement(
                        ticks_group, "text", x=repr(x), y=repr(y), style=_css_style(dstyle))
                    label_xml.text = label
                    if axes.y.ticks.labels._angle:
                        label_xml.set(
                            "transform", "rotate(%r, %r, %r)" % (-axes.y.ticks.labels._angle, x, y))
                    if "-toyplot-anchor-shift" in dstyle:
                        label_xml.set(
                            "dx", str(dstyle["-toyplot-anchor-shift"]))
                    if title is not None:
            ...
            if axes.y.label._text is not None:
                ...
                xml.SubElement(axes_xml, "text", x=repr(x), y=repr(
                    y), transform="rotate(-90, %r, %r)" % (x, y), style=_css_style(axes.y.label._style)).text = axes.y.label._text

        if axes.label._text is not None:
            x = (axes._xmin_range + axes._xmax_range) * 0.5
            y = axes._ymin_range
            xml.SubElement(axes_xml, "text", x=repr(x), y=repr(
                y), style=_css_style(axes.label._style)).text = axes.label._text
...
def _render(canvas, axes, context):
    ...
    for cell in axes._visible_cells:
        if cell._bstyle is not None:
            cell_xml = xml.SubElement(axes_xml, "rect", x=repr(cell._left), y=repr(cell._top), width=repr(
                cell._right - cell._left), height=repr(cell._bottom - cell._top), style=_css_style(cell._bstyle))
        ...
        if cell._align == "left":
            ...
            xml.SubElement(axes_xml, "text", x=repr(x), y=repr(y), style=_css_style(
                toyplot.style.combine(cell._style, {"text-anchor": "begin"}))).text = prefix + separator + suffix
        elif cell._align == "center":
            x = column_center + cell._column_offset
            xml.SubElement(axes_xml, "text", x=repr(x), y=repr(y), style=_css_style(
                toyplot.style.combine(cell._style, {"text-anchor": "middle"}))).text = prefix + separator + suffix
        elif cell._align == "right":
            x = column_right + cell._column_offset
            xml.SubElement(axes_xml, "text", x=repr(x), y=repr(y), style=_css_style(
                toyplot.style.combine(cell._style, {"text-anchor": "end"}))).text = prefix + separator + suffix
        elif cell._align is "separator":
            x = column_center + cell._column_offset

            xml.SubElement(axes_xml, "text", x=repr(x - 2), y=repr(y), style=_css_style(
                toyplot.style.combine(cell._style, {"text-anchor": "end"}))).text = prefix
            xml.SubElement(axes_xml, "text", x=repr(x), y=repr(y), style=_css_style(
                toyplot.style.combine(cell._style, {"text-anchor": "middle"}))).text = separator
            xml.SubElement(axes_xml, "text", x=repr(x + 2), y=repr(y), style=_css_style(
                toyplot.style.combine(cell._style, {"text-anchor": "begin"}))).text = suffix
    ...
    for row_index, row in enumerate(hlines):
        ...
        for start, end, line_type in contiguous(row):
            if line_type == "single":
                xml.SubElement(axes_xml, "line", x1=repr(column_boundaries[start]), y1=repr(
                    y), x2=repr(column_boundaries[end]), y2=repr(y), style=_css_style(axes._gstyle))
            elif line_type == "double":
                xml.SubElement(axes_xml, "line", x1=repr(column_boundaries[start]), y1=repr(
                    y - separation), x2=repr(column_boundaries[end]), y2=repr(y - separation), style=_css_style(axes._gstyle))
                xml.SubElement(axes_xml, "line", x1=repr(column_boundaries[start]), y1=repr(
                    y + separation), x2=repr(column_boundaries[end]), y2=repr(y + separation), style=_css_style(axes._gstyle))
    ...
    for column_index, column in enumerate(vlines.T):
        ...
        for start, end, line_type in contiguous(column):
            if line_type == "single":
                xml.SubElement(axes_xml, "line", x1=repr(x), y1=repr(row_boundaries[start]), x2=repr(
                    x), y2=repr(row_boundaries[end]), style=_css_style(axes._gstyle))
            elif line_type == "double":
                xml.SubElement(axes_xml, "line", x1=repr(x - separation), y1=repr(row_boundaries[start]), x2=repr(
                    x - separation), y2=repr(row_boundaries[end]), style=_css_style(axes._gstyle))
                xml.SubElement(axes_xml, "line", x1=repr(x + separation), y1=repr(row_boundaries[start]), x2=repr(
                    x + separation), y2=repr(row_boundaries[end]), style=_css_style(axes._gstyle))
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-BarBoundaries"})
    ...
    for boundary1, boundary2, fill, opacity, title in zip(boundaries.T[:-1], boundaries.T[1:], [mark._table[key] for key in mark._fill], [mark._table[key] for key in mark._opacity], [mark._table[key] for key in mark._title]):
        not_null = numpy.invert(numpy.logical_or(
            numpy.ma.getmaskarray(boundary1), numpy.ma.getmaskarray(boundary2)))
        ...
        for dleft, dright, dboundary1, dboundary2, dfill, dopacity, dtitle in zip(left[not_null], right[not_null], boundary1[not_null], boundary2[not_null], fill[not_null], opacity[not_null], title[not_null]):
            ...
            datum_xml = xml.SubElement(series_xml, "rect", attrib={"class": "toyplot-Datum", axis1: repr(min(dleft, dright)), axis2: repr(
                min(dboundary1, dboundary2)), distance1: repr(numpy.abs(dleft - dright)), distance2: repr(numpy.abs(dboundary1 - dboundary2))}, style=_css_style(dstyle))
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-BarMagnitudes"})
    context.add_data_table(mark, mark._table, title="Bar Data")
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-FillBoundaries"})
    ...
    for boundary1, boundary2, fill, opacity, title in zip(boundaries.T[:-1], boundaries.T[1:], mark._fill, mark._opacity, mark._title):
        ...
        for segment in segments:
            if mark._position_axis == "x":
                coordinates = zip(numpy.concatenate((position[segment], position[segment][
                                  ::-1])), numpy.concatenate((boundary1[segment], boundary2[segment][::-1])))
            elif mark._position_axis == "y":
                coordinates = zip(numpy.concatenate((boundary1[segment], boundary2[segment][
                                  ::-1])), numpy.concatenate((position[segment], position[segment][::-1])))
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-FillMagnitudes"})
    context.add_data_table(mark, mark._table, title="Fill Data")
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-AxisLines"})
    series_xml = xml.SubElement(
        mark_xml, "g", attrib={"class": "toyplot-Series"})
    for dposition, dstroke, dopacity, dtitle in zip(position, mark._table[mark._stroke[0]], mark._table[mark._opacity[0]], mark._table[mark._title[0]]):
        ...
        datum_xml = xml.SubElement(series_xml, "line", attrib={"class": "toyplot-Datum", p1: repr(
            dposition), p2: repr(dposition), b1: repr(boundary1), b2: repr(boundary2)}, style=_css_style(dstyle))
        if dtitle is not None:
            xml.SubElement(datum_xml, "title").text = str(dtitle)
...
def _render(canvas, legend, context):
    ...
    xml.SubElement(context.root, "rect", x=repr(x), y=repr(y), width=repr(width), height=repr(
        height), style=_css_style(legend._style), id=context.get_id(legend), attrib={"class": "toyplot-mark-Legend"})
    ...
    if mark_count:
        ...
        for i, mark in enumerate(legend._marks):
            ...
            if mark_type == "line":
                xml.SubElement(context.root, "line", x1=repr(mark_x), y1=repr(
                    mark_y + mark_height), x2=repr(mark_x + mark_width), y2=repr(mark_y), style=_css_style(mark_style))
            elif mark_type == "rect":
                xml.SubElement(context.root, "rect", x=repr(mark_x), y=repr(mark_y), width=repr(
                    mark_width), height=repr(mark_height), style=_css_style({"stroke": "none"}, mark_style))
            elif isinstance(mark_type, toyplot.mark.Plot):
                dstyle = toyplot.style.combine(
                    {"stroke": toyplot.color.to_css(mark_type._stroke[0])}, mark_type._style)
                xml.SubElement(context.root, "line", x1=repr(mark_x), y1=repr(
                    mark_y + mark_height), x2=repr(mark_x + mark_width), y2=repr(mark_y), style=_css_style(dstyle, mark_style))
                ...
                xml.SubElement(context.root, "rect", x=repr(mark_x), y=repr(mark_y), width=repr(
                    mark_width), height=repr(mark_height), style=_css_style(dstyle, mark_style))
            else:
                computed_style = toyplot.style.combine(
                    {"stroke": toyplot.color.near_black, "fill": "none"}, mark_style)
                _render_marker(context.root, mark_x + (mark_width / 2), mark_y + (
                    mark_height / 2), min(mark_width, mark_height), mark_type, computed_style, {})

            label_x = x + mark_width + (2 * mark_gutter)
            label_y = y + ((i + 1) * mark_gutter) + \
                (i * mark_height) + (mark_height / 2)
            xml.SubElement(context.root, "text", x=repr(label_x), y=repr(label_y), style=_css_style(
                {"alignment-baseline": "middle", "stroke": "none"}, legend._lstyle)).text = mark_label
...
def _render(axes, mark, context):
    ...
    if mark._show_edges:
        ...
        for source, target, dstroke, dstroke_width, dstroke_opacity in itertools.izip(mark._edge_table[mark._source[0]], mark._edge_table[mark._target[0]], mark._edge_table[mark._stroke[0]], mark._edge_table[mark._stroke_width[0]], mark._edge_table[mark._stroke_opacity[0]]):
            edge_style = toyplot.style.combine({"stroke": toyplot.color.to_css(
                dstroke), "stroke-width": dstroke_width, "stroke-opacity": dstroke_opacity}, mark._edge_style)
            xml.SubElement(edge_xml, "line", x1=repr(x[source]), y1=repr(y[source]), x2=repr(
                x[target]), y2=repr(y[target]), style=_css_style(edge_style))
    ...
    for dx, dy, dmarker, dsize, dfill, dstroke, dopacity in itertools.izip(x, y, marker, msize, mfill, mstroke, mopacity):
        dstyle = toyplot.style.combine({"fill": toyplot.color.to_css(
            dfill), "stroke": toyplot.color.to_css(dstroke), "opacity": dopacity}, mark._mstyle)
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-Plot"})
    ...
    for series, stroke, stroke_width, stroke_opacity, title, marker, msize, mfill, mstroke, mopacity in zip(series.T, mark._stroke.T, mark._stroke_width.T, mark._stroke_opacity.T, mark._title.T, [mark._table[key] for key in mark._marker], [mark._table[key] for key in mark._msize], [mark._table[key] for key in mark._mfill], [mark._table[key] for key in mark._mstroke], [mark._table[key] for key in mark._mopacity]):
        ...
        msize = numpy.sqrt(msize)
        stroke_style = toyplot.style.combine({"stroke": toyplot.color.to_css(
            stroke), "stroke-width": stroke_width, "stroke-opacity": stroke_opacity}, mark._style)
        if mark._coordinate_axes[0] == "x":
            x = position
            y = series
        elif mark._coordinate_axes[0] == "y":
        ...
        if mark._show_stroke:
            ...
            xml.SubElement(
                series_xml, "path", d=" ".join(d), style=_css_style(stroke_style))
...
def _render(axes, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-Rect"})
    ...
    for dx1, dx2, dy1, dy2, dfill, dopacity, dtitle in zip(x1, x2, y1, y2, mark._table[mark._fill[0]], mark._table[mark._opacity[0]], mark._table[mark._title[0]]):
        dstyle = toyplot.style.combine(
            {"fill": toyplot.color.to_css(dfill), "opacity": dopacity}, mark._style)
        datum_xml = xml.SubElement(series_xml, "rect", attrib={"class": "toyplot-Datum"}, x=repr(min(dx1, dx2)), y=repr(
            min(dy1, dy2)), width=repr(numpy.abs(dx1 - dx2)), height=repr(numpy.abs(dy1 - dy2)), style=_css_style(dstyle))
        if dtitle is not None:
            xml.SubElement(datum_xml, "title").text = str(dtitle)
...
def _render(parent, mark, context):
    ...
    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-Text"})
    ...
    for dx, dy, dtext, dangle, dfill, dopacity, dtitle in zip(x, y, mark._table[mark._text[0]], mark._table[mark._angle[0]], mark._table[mark._fill[0]], mark._table[mark._opacity[0]], mark._table[mark._title[0]]):
        dstyle = toyplot.style.combine(
            {"fill": toyplot.color.to_css(dfill), "opacity": dopacity}, mark._style)
        datum_xml = xml.SubElement(series_xml, "text", attrib={"class": "toyplot-Datum"}, x=repr(
            dx), y=repr(dy), transform="rotate(%r, %r, %r)" % (-dangle, dx, dy), style=_css_style(dstyle))
        if "-toyplot-anchor-shift" in dstyle:
...
def _render(canvas, mark, context):
    ...
    swatches = numpy.linspace(
        ymin - (swatch_size / 2), ymax + (swatch_size / 2), len(samples) + 1, endpoint=True)

    mark_xml = xml.SubElement(context.root, "g", style=_css_style(
        mark._style), id=context.get_id(mark), attrib={"class": "toyplot-mark-VColorBar"})
    for sample, y1, y2 in zip(samples, swatches[:-1], swatches[1:]):
        color = mark._project_color(sample)
        xml.SubElement(mark_xml, "rect", x=repr(xmin), y=repr(y1), width=repr(xmax - xmin), height=repr(
            (y2 - y1) + (swatch_size / 2)), style=_css_style({"stroke": "none", "fill": toyplot.color.to_css(color)}))
    ...
    if mark.ticks._show:
        ...
        for location, tick_style in zip(mark._tick_locations, mark.ticks.tick.styles(mark._tick_locations)):
            ...
            xml.SubElement(ticks_group, "line", x1=repr(x1), y1=repr(y), x2=repr(
                x2), y2=repr(y), style=_css_style(mark.ticks._style, tick_style))
    ...
    if mark.ticks.labels._show:
        ...
        for location, label, title, label_style in zip(mark._tick_locations, mark._tick_labels, mark._tick_titles, mark.ticks.labels.label.styles(mark._tick_locations)):
            ...
            y = mark._project_y(location)
            label_xml = xml.SubElement(ticks_group, "text", x=repr(x), y=repr(y), transform="rotate(-90, %r, %r)" % (x, y), style=_css_style(
                {"text-anchor": "middle", "alignment-baseline": "middle", "baseline-shift": "-80%"}, mark.ticks.labels._style, label_style))
            label_xml.text = label
            if title is not None:
                xml.SubElement(label_xml, "title").text = str(title)
    ...
    if mark.label._text is not None:
        ...
        xml.SubElement(mark_xml, "text", x=repr(x), y=repr(y), transform="rotate(-90, %r, %r)" %
                       (x, y), style=_css_style(mark.label._style)).text = mark.label._text
